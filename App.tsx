...
const App: React.FC = () => {
  ...
  // Add Drink State
  const [addTab, setAddTab] = useState<'brand' | 'manual' | 'ai'>('brand');
  ...
  const renderAdd = () => (
    <div className="pb-24 space-y-6">
      ...
      <div className="flex space-x-1 bg-slate-800 p-1 rounded-xl">
          <button 
             onClick={() => { setAddTab('brand'); setSelectedBrand(null); setSelectedDrink(null); }}
             className={`flex-1 py-2 text-sm font-medium rounded-lg transition ${addTab === 'brand' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}
          >
            品牌資料庫
          </button>
          <button 
             onClick={() => { setAddTab('manual'); }}
             className={`flex-1 py-2 text-sm font-medium rounded-lg transition ${addTab === 'manual' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}
          >
            手動輸入
          </button>
          <button 
             onClick={() => { setAddTab('ai'); setScanResult(null); }}
             className={`flex-1 py-2 text-sm font-medium rounded-lg transition ${addTab === 'ai' ? 'bg-slate-700 text-white shadow' : 'text-slate-400 hover:text-slate-200'}`}
          >
            AI 辨識
          </button>
      </div>
      ...
      <div className="mt-4">
        ...
        {/* Brand Selector View - 3 Level Depth */}
        {addTab === 'brand' && (
  ...
  const renderStats = () => {
    // Group logs by day (mock logic for demo: showing hourly trend of today instead)
    ...
    return (
      <div className="pb-24 space-y-6">
        ...
        <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
           <h3 className="text-sm font-medium text-slate-400 mb-4">攝取歷史趨勢</h3>
           <div className="h-64 w-full">
             <ResponsiveContainer width="100%" height="100%">
               <BarChart data={data}>
                 ...
                 />
                 <Bar dataKey="mg" fill="#6366f1" radius={[4, 4, 0, 0]}>
                    {data.map((entry, index) => (
                      <Cell key={`cell-${index}`} fill={entry.mg > 200 ? '#ef4444' : '#6366f1'} />
                    ))}
                 </Bar>
               </BarChart>
             </ResponsiveContainer>
        ...
        <div className="grid grid-cols-2 gap-4">
           <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
             <div className="text-slate-400 text-xs uppercase">今日總攝取</div>
             <div className="text-2xl font-bold text-white mt-1">
               {logs.reduce((acc, curr) => acc + curr.amountMg, 0)} <span className="text-sm font-normal text-slate-500">mg</span>
             </div>
           </div>
           <div className="bg-slate-800 p-4 rounded-xl border border-slate-700">
             <div className="text-slate-400 text-xs uppercase">每日上限</div>
             <div className="text-2xl font-bold text-white mt-1">
               {settings.dailyLimitMg} <span className="text-sm font-normal text-slate-500">mg</span>
             </div>
           </div>
        </div>
      </div>
    ...
    <HashRouter>
      <div className="min-h-screen bg-slate-900 text-slate-50 p-6 selection:bg-indigo-500/30">
        <main className="max-w-md mx-auto relative">
          ...
          {currentTab === TabView.STATS && renderStats()}
